...
pymysql.install_as_MySQLdb()  # 关键：模拟 MySQLdb 接口
__all__ = [
    'create_session',
    'DBConfigParser',
    'ConnBase',
...
class SQLAlchemy(ConnBase):
    def __init__(self, config=None, master_url=None, slaves_url=None, session_conf=None, **kwargs):
        ...
        self._master_engine = engine_from_config(self.config,prefix=_SQLALCHEMY_PREFIX,url=master_url[0],**filtered_kwargs)
        self._master_session = create_session(self._master_engine, **session_conf)
        self._slaves_session = []
        self._slave_engine = []

        for slave in slaves_url:
            slave_engine = engine_from_config(self.config, prefix=_SQLALCHEMY_PREFIX, url=slave, **filtered_kwargs)
